...
## 12. Implementation Log — OPEN gaps closed (2026-09-23, second pass)

All ⚪ OPEN items that required only repo work are now implemented. Remaining ⚪ items need your input (pricing numbers, iCare stats, external event URLs) or hosting access.

| Gap (audit ref) | Implementation | Files |
|---|---|---|
| §9.1 Speaking page (entity-authority gap: talks self-attested only) | New page with 4 event cards, photos, TL;DR summary, invite CTA, and full `Event` JSON-LD (location, organizer, audience, image per event); index nav "Talks" now points here | `speaking.html` (new) |
| §9.4 Glossary (definitional AEO queries) | New page: 15 quotable definitions (ABDM, M1/M2/M3, ABHA, HIP/HIU, FHIR R4, Care Context, Consent Artifact, FEFO, Cold Chain, RLS, Schema-per-Tenant, DPDP, Data Fiduciary, Double-Entry Ledger, Offline-Tolerant), each with `#anchor`, cross-links to essays + official sources, `DefinedTermSet` JSON-LD, TOC | `glossary.html` (new) |
| §9.5 Company page (SoftEdge had no entity page here) | New page: company facts card, 6 service lines, 3 production deployments (iCare/TaskEdge/PSACS), engagement routing; full `Organization` JSON-LD (address, contactPoint, knowsAbout, OfferCatalog, founder/employee edges, sameAs) | `company.html` (new) |
| §4.2 Hero PNG 2.05 MB | Re-exported: 880×1140 adaptive-palette PNG → **464 KB** (−77%); WebP twin regenerated at 880×1140 → **72 KB**; intrinsic dims updated in HTML/meta | `images/pravesh-singh.png`, `.webp`, `index.html`, `expertise.html` |
| §4.2 CLS on gallery images | `width`/`height` added to all showcase `<img>` tags (11 images, real intrinsic dims) | `index.html` (+ new pages ship with dims) |
| §5.2 Organization entity thin | `description`, `address`, `contactPoint` (sales, email, availableLanguage), `sameAs` → company page added to Organization node | `index.html` JSON-LD |
| §1.3e AI-crawler measurement (no server access from repo) | Ready-to-run log analysis script: per-bot hit tallies (17 AI bots + Googlebot/bingbot reference) + top-10 paths crawled by AI agents; usage for Apache/Nginx logs documented in header | `tools/ai_crawler_report.sh` (new) |
| §3 Sitemap coverage | 3 new URLs registered (17 total) | `sitemap.xml` |
| §5 Machine discovery of new pages | llms.txt: new "Speaking & Public Demonstrations" section (4 dated events), "Glossary" section, navigation entries; llms-full.txt §9 endpoints updated | `llms.txt`, `llms-full.txt` |
| Internal linking | Footer links to Speaking/Glossary/Company added on all 13 existing pages + writing template; nav "Talks" → `speaking.html` | all pages |

### Verification (all PASS)

- JSON-LD valid on **all 19 HTML files** (incl. 3 new pages)
- `sitemap.xml` valid XML, 17 URLs; `writing/feed.xml` valid Atom
- New pages each have: unique canonical, OG/Twitter cards, favicon links, Atom link, single H1, full JSON-LD
- Build mirror resynced and in parity

### Still open (needs you / hosting)

| Item | Blocking dependency |
|---|---|
| Public pricing + `Offer.priceSpecification` | Your pricing decision (§9.3) |
| Case-study outcomes page with real stats | Your numbers: patients served, hospitals live, vials tracked, stock-out reduction (§9.2) |
| External links on speaking page (event programs, news mentions) | URLs from you; they convert self-attested events into third-party-verified facts |
| `sameAs` expansion (GitHub/X/Wikidata) | Profiles must exist first |
| AI-crawler report first run | Server access: `bash tools/ai_crawler_report.sh /var/log/apache2/access.log` |
| **Deploy** | Everything above ships together — deploy repo root, then IndexNow ping + Rich Results validation (§11 P0/P1) |
